...
#define             MEMORY_INVALID_ADDRESS 0xffffffffffffULL
...
memory_add_pending(uint64_t address, uint64_t size_in_bytes, int op) {
    ...
    for (int i = 0; i < MEMORY_MAX_PENDING; ++i, ++pnd) {
        if (pnd->op == MEMORY_OP_NONE || pnd->op == MEMORY_OP_COMPLETED) {
            pnd->address = address;
...
bool memory_status(uint64_t address, void *value) {
    for (int i = 0; i < MEMORY_MAX_PENDING; ++i) {
        if (memory_pending[i].address == address && memory_pending[i].op != MEMORY_OP_NONE) {
